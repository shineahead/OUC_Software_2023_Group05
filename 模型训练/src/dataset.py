...
class MyDataset_source(Dataset):
    def __init__(self, mode='train'):
        ...
        if mode == 'train':
            ...
            self.labels = scipy.io.loadmat(os.path.join(os.getcwd(), 'data/Training_Label_Florence.mat'))[
                'Training_Label']
            self.data = self.data.transpose((0, 3, 1, 2))

        elif mode == 'test':
            self.data = scipy.io.loadmat(os.path.join(os.getcwd(), 'data/Testing_Data_Florence.mat'))['Testing_Data']
            self.labels = scipy.io.loadmat(os.path.join(os.getcwd(), 'data/Testing_Label_Florence.mat'))[
                'Testing_Label']
            self.data = self.data.transpose((0, 3, 1, 2))
...
class MyDataset_target(Dataset):
    def __init__(self, mode='train'):
        ...
        if mode == 'train':
            ...
            self.labels = scipy.io.loadmat(os.path.join(os.getcwd(), 'data/Training_Label_ottawa.mat'))[
                'Training_Label']
            self.data = self.data.transpose((0, 3, 1, 2))
